...
void FEM::init(const function2D & _u, const function2D & _f, const function2D &_lambda, double _sigma, bool _isGridUniform, bool _isTimeUniform, int _condType, int _coefGrid, int _coefTime)
{
...
pair<int, double> FEM::solve()
{
	...
	return make_pair(count, calcNormAtMainNodes(q));
}
...
{
	double arg = (q[elemNumber + 1] - q[elemNumber]) / hx;
	lambda0 = lambda(arg, nodes[elemNumber].x);
	lambda1 = lambda(arg, nodes[elemNumber+1].x);
	double numerator = (lambda0 + lambda1) / hx;
...
{
	bLocal = { 0, 0 };
	bLocal[0] = hx * (2 * f(nodes[elemNumber].x, t) + f(nodes[elemNumber + 1].x, t)) / 6
		+ sigma * hx * (2 * qPrev[elemNumber] + qPrev[elemNumber + 1]) / (6 * dt);
	bLocal[1] = hx * (f(nodes[elemNumber + 1].x, t) + 2 * f(nodes[elemNumber + 1].x, t)) / 6
		+ sigma * hx * (qPrev[elemNumber] + 2 * qPrev[elemNumber + 1]) / (6 * dt);
}